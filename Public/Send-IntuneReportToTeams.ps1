function Send-IntuneReportToTeams {
    ...
    .DESCRIPTION
        ...
        scheduled task (pair it with Connect-JustGraphIT app-only auth).
    ...
    .PARAMETER WebhookUrl
        The Workflows incoming-webhook URL. Defaults to $env:JUSTGRAPHIT_TEAMS_WEBHOOK so it
    ...
    param(
        ...
        [string]$WebhookUrl = $env:JUSTGRAPHIT_TEAMS_WEBHOOK,
        [string]$Summary,
        [string[]]$Column,
    ...
    process { foreach ($o in $InputObject) { if ($null -ne $o) { [void]$rows.Add($o) } } }
        ...
        if ([string]::IsNullOrWhiteSpace($WebhookUrl)) {
            throw "No Teams webhook URL. Pass -WebhookUrl or set `$env:JUSTGRAPHIT_TEAMS_WEBHOOK to a Power Automate 'Workflows' incoming-webhook URL."
        }
