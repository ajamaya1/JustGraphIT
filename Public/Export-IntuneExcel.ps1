function Export-IntuneExcel {
    <#
    .SYNOPSIS
        Export any JUSTGRAPHIT output to a beautifully-formatted Excel workbook.
    ...
    .EXAMPLE
        Get-IntuneAssignment -Flat | Export-IntuneExcel -Path JUSTGRAPHIT.xlsx -WorksheetName Assignments -Title 'All assignments'

        A formatted assignments sheet with exclude/unassigned coloring.

    .EXAMPLE
        Get-IntuneDeploymentSummary -FailuresOnly | Export-IntuneExcel -Path JUSTGRAPHIT.xlsx -WorksheetName Failures -Append
        Get-IntuneAssignmentDrift -Path base.json | Export-IntuneExcel -Path JUSTGRAPHIT.xlsx -WorksheetName Drift -Append -Show
